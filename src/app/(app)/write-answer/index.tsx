...
import { InputAlertModal } from "../../../components/input-alert-modal";
...
`.trim();
  ...
  const handleAiOk = async (value: string) => {
    ...
    const resetMinutes = Math.max(Math.ceil((RESET_TIME - elapsed) / 60000), 0);
    setAiInfo({ remaining, resetMinutes });
    setTooltipVisible(true);
  };

  const handleAiCancel = () => setAiModalVisible(false);
